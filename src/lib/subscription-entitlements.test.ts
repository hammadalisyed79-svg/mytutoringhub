...
// Priority Verification Review SKU alone does not buy ranking tier or badge.
assert.equal(computeTutorPlanTier(new Set(["VERIFIED_TUTOR"])), 0);
assert.equal(computeTutorPlanTier(new Set(["VERIFIED_TUTOR", "TUTOR_BASIC"])), 1);
assert.equal(computeTutorPlanTier(new Set(), { identityVerified: true }), 2);
assert.equal(
  computeTutorPlanTier(new Set(["TUTOR_BASIC"]), { identityVerified: true }),
  2,
);
