...
export function hubPointsRedeemCards(role: string): HubPointsRedeemCard[] {
  if (role === "TUTOR") {
    return [
      {
        title: "Tutor Pro",
        description: "Up to 10 Teaching Profiles, ranking, and unlimited student contacts",
      ...
      {
        title: "Priority Verification Review",
        description: "Jump the identity-verification queue (badge earned only after approval)",
        href: "/pricing?plan=VERIFIED_TUTOR",
      },
    ];
  }
  return [
...
function hubPointsRedeemHints(role: string) {
  return role === "TUTOR"
    ? ["Tutor Pro", "Listing Boost", "Priority Verification Review"]
...
export async function tryAwardProfileCompleteBonus(userId: string) {
  ...
  if (
    !isTutorProfileListable(
      { ...user.tutorProfile, subjectProfiles: user.tutorProfile.subjectProfiles },
      user.name,
    )
  ) {
    return;
